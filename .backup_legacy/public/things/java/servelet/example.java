...
//		System.out.println(n + " " + s);
		...
		try {
			...
			String dbURL = "jdbc:mysql://localhost:3306/095";
			String dbUser = "root";
			String dbPass = "";
			...
			if(n == 1) {//area
				String query = "select phone_no from police_station where area_name = \"" + s + "\"";
				rs = st.executeQuery(query);
				while(rs.next()) {
					out.print("Area : " + s + ", Phone No: " + rs.getInt(1));
				}
			} else { //phone
				String query = "select area_name from police_station where phone_no = \"" + s + "\"";
				rs = st.executeQuery(query);
				while(rs.next()) {
